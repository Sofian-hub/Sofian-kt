...
function bruteforce() {
    ...
    // Boucle de bruteforce
    const delay = 50; // Délai entre les tests
            ...
            for (let i2 = 1; i2 <= maxValues[1]; i2++) {
                for (let i3 = 1; i3 <= maxValues[2]; i3++) {
                    for (let i4 = 1; i4 <= maxValues[3]; i4++) {
                        if (found) return; // Sortir si déjà trouvé
                        ...
                        setTimeout(() => {
                            ...
                            if (verifierFichierExiste(fileName)) {
                                ...
                                const countdownInterval = setInterval(() => {
                                    ...
                                    if (countdown <= 0) {
                                        ...
                                        window.location.href = "contact.html"; // Redirection
                                    }
                            ...
                            // Vérifiez si tous les tests ont été effectués
                            if (total === maxValues[0] * maxValues[1] * maxValues[2] * maxValues[3] && !found) {
                                spinner.classList.add("hidden");
                                questionDiv.textContent = "redirection en cours ⌛";
                                setTimeout(() => {
                                    window.location.href = "contact.html"; // Redirection finale si rien trouvé
